package com.projectiyzico.controller;
...
import com.projectiyzico.service.ExceptionHandler;
import com.projectiyzico.service.PaymentService;
...
public class IndexController {
        ...
        if(session.isNew()){
            ...
            if(session.getAttribute("basketItems") ==null || session.getAttribute("basketItems").equals("")){
                ...
                basketItems.forEach(basketItem -> {
                    System.out.println("ÜRÜN ADI : " + basketItem.getName());
                });
                modelAndView.addObject("toplam",priceSum(basketItems));
            }
            System.out.println("VAROLAN SESSION");
        }
    ...
    public String errorParams(@RequestParam Map<String,String> allRequestParams){
    ...
    public ModelAndView paymentSend(@RequestParam Map<String,String> params, @SessionAttribute("basketItems") List<BasketItem> basketItems, HttpServletRequest request) throws JsonProcessingException, PropertiesNotFound, UnknownHostException {
        ...
        paymentRequest.setCallbackUrl("http://localhost:8080/callback");
        ...
        System.out.println("SESSION TOPLAM : " + priceSum(basketItems));

        //basketItems.get(0).setPrice(new BigDecimal(params.get("totalPrice")));
        ...
        if(paymentState.getStatus().equals("failure")){
            String stringBuilder="?name=".concat(buyer.getName()).concat("&surname=").concat(buyer.getSurname())
                    ...
                    .concat(shippingAddress.getCountry()).concat("&address=").concat("&zipCode=").concat(shippingAddress.getZipCode()).concat(shippingAddress.getAddress())
